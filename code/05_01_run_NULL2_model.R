...
# this is the first run of the NULL2 (sinusoidal forcing of R0) model with the original 5000 parameter combinations using bounds from literature
...
for (d in 1:length(districts_list)) {
  ...
  num_ens=dim(parms)[1]
  R0 = calc_R0_sinusoidal_ens(in.parms = parms, num.ens = num_ens, sh1 = q_ts)
  ...
  saveRDS(list(out_monthly_all, out_df_top_ten, mean_rmse_df, top_ten_ens, out), file = here("out", "sim_results", paste0(district_name, "_numsim_",num_sim,"_topten_NULL2.RDS")))
