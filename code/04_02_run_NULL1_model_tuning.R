...
parms <- readRDS(here("data", "cleaned", "LHS_5000_parameter_matrix_NULL1.RDS"))
...
for (d in 1:length(districts_list_limited)) {
  ...
  tmp <- readRDS(file = here("out", "sim_results", paste0(district_name, "_numsim_",num_sim,"_topten_NULL1.RDS"))) #original simulation
  mean_rmse_df <- tmp[[3]] 
...
lowerbound <- c(1.5, 2, 365, 0.4, 500) #original bounds
upperbound <- c(3, 5, 356*4, 0.8, 1000)
names(lowerbound) <- names(upperbound) <- c("R0max", "D", "L", "S0perc", "I0")
...
hdi_range$parameter[hdi_range$lower_perc_change >= perc_change_bound][!is.na(hdi_range$parameter[hdi_range$lower_perc_change >= perc_change_bound])] #D, L
hdi_range$parameter[hdi_range$upper_perc_change >= perc_change_bound] #R0max

saveRDS(hdi_range, file = here("data", "cleaned", paste0("hdi_range_tuning_round",tuning_round,"_NULL1.RDS")))
...
saveRDS(list(lowerbound, upperbound), file = here("data", "cleaned", paste0("LHS_tuning_round",tuning_round,"_parameter_matrix_updatedbounds_NULL1.RDS")))
...
y[,1] <- qunif(x[,1], 1.5, hdi_range$upper[hdi_range$parameter == "R0max"]) #R0max 
y[,2] <- qunif(x[,2],  hdi_range$lower[hdi_range$parameter == "D"], 5) #D
y[,3] <- qunif(x[,3],  hdi_range$upper[hdi_range$parameter == "L"], 365*4) #L 
y[,4] <- qunif(x[,4], 0.4, 0.8) #S0perc
y[,5] <- qunif(x[,5], 500,1000) #I0

parms <- y %>% as.data.frame()
names(parms) <- c("R0max",  "D", "L", "S0perc", "I0")
saveRDS(parms, file = here("data", "cleaned", paste0("LHS_",num_ens,"_parameter_matrix_tuning_round",tuning_round,"_NULL1.RDS")))
...
perc_change_bound = 10
num_ens_keep = 500
parms <- readRDS(here("data", "cleaned", paste0("LHS_5000_parameter_matrix_tuning_round",tuning_round - 1,"_NULL1.RDS")))
districts_list_limited <- readRDS(here("data", "cleaned", "district_list_limited.RDS"))
...
for (d in 1:length(districts_list_limited)) {
  ...
  # set name of district
  district_name = districts_list_limited[d]
  tmp <- readRDS(file = here("out", "sim_results", paste0(district_name, "_numsim_",num_sim,"_topten_tuning_round",tuning_round,"_NULL1.RDS")))
  mean_rmse_df <- tmp[[3]] 
  top_ens <- mean_rmse_df$num_ens[1:num_ens_keep]
...
hdi_range <- hdi_range %>% filter(!is.na(parameter))
lowerbound = readRDS(file = here("data", "cleaned", paste0("LHS_tuning_round",tuning_round-1,"_parameter_matrix_updatedbounds_NULL1.RDS")))[[1]]
upperbound = readRDS(file = here("data", "cleaned", paste0("LHS_tuning_round",tuning_round-1,"_parameter_matrix_updatedbounds_NULL1.RDS")))[[2]]
