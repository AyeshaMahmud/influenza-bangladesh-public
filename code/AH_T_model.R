...
#### calculate R0 ####
calc_R0_sinusoidal_ens <- function(in.parms, num.ens, sh1){
  # fixed parameters
  t = seq(1:length(sh1))
  omega = 2*pi/365
  phi = 270
  ...
  R0  = matrix(0, length(sh1), num.ens)
  
  # Loop through all ensemble members:
  for (ix in 1:num.ens) {
    R0max = in.parms[ix, 'R0max']
    lambda = in.parms[ix, 'lambda']
    
    # Store results in matrices:
    R0[, ix] <- R0max*(1+lambda*sin(omega*(t+phi)))
  }
  
  return(R0)
}

calc_R0_AH_T_ens <- function(in.parms, num.ens, sh1, mean.temp) {
...
# SIRS model adpated from https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1009050#pcbi-1009050-t001
# https://github.com/wan-yang/flu-subtropic-climate-models/blob/main/model_code/AH_T.R
SIRS_AH_T <- function(tm_strt, tm_end, tm_step, S0, I0, N, D, L, mu, beta, expoI, realdata=FALSE){
  ...
  #         D: infection period, day; L: immune period, day; mu: birth and death rate
  #         expoI = alpha: introduce nonlinearity into the infection process (i.e., imperfect population mixing); beta: transmission rate
