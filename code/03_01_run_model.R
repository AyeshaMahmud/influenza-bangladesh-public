...
## Note: this is the first run of the model with the original 5000 parameter combinations 
# Assumes population data from worldpop is downloaded and saved in data > district_pop.csv
...
for (d in 1:length(districts_list)) {
  ...
  saveRDS(list(out_monthly_all, out_df_top_ten, mean_rmse_df, top_ten_ens, out), file = here("out", "sim_results", paste0(district_name, "_numsim_",num_sim,"_topten.RDS")))
