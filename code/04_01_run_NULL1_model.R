...
# this is the first run of the NULL1 (constant R0) model with the original 5000 parameter combinations using bounds from literature
# for final results, use final run of the model using 10,000 paramter combinations using bounds after parameter tuning
...
#for (d in c(1,4,5,12)) {
  ...
  num_ens=dim(parms)[1]
  #R0 = calc_R0_AH_T_ens(in.parms = parms, num.ens = num_ens, sh1 = q_ts, mean.temp = T_ts)[[2]]
  ...
  saveRDS(list(out_monthly_all, out_df_top_ten, mean_rmse_df, top_ten_ens, out), file = here("out", "sim_results", paste0(district_name, "_numsim_",num_sim,"_topten_NULL1.RDS")))
